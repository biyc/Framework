...
namespace ETHotfix
{
    public sealed class EventSystem
    {
        ...
        private readonly UnOrderMultiMap<Type, IOnGUISystem> OnGUISystems = new UnOrderMultiMap<Type, IOnGUISystem>();
        ...
        private Queue<long> onguis = new Queue<long>();
        private Queue<long> onguis2 = new Queue<long>();
        ...
        public EventSystem()
        {
            ...
            foreach (Type type in types)
            {
                ...
                IOnGUISystem OnGUISystem = obj as IOnGUISystem;
                if (OnGUISystem != null)
                {
                    this.OnGUISystems.Add(OnGUISystem.Type(), OnGUISystem);
                }
        ...
        public void Add(Component component)
        {
            ...
            if (this.OnGUISystems.ContainsKey(type))
            {
                this.onguis.Enqueue(component.InstanceId);
            }
        ...
        public void OnGUI()
        {
            while (this.onguis.Count > 0)
            {
                long instanceId = this.onguis.Dequeue();
                Component component;
                if (!this.allComponents.TryGetValue(instanceId, out component))
                {
                    continue;
                ...
                List<IOnGUISystem> iOnGUISystems = this.OnGUISystems[component.GetType()];
                if (iOnGUISystems == null)
                {
                    continue;
                }

                this.onguis2.Enqueue(instanceId);

                foreach (IOnGUISystem iOnGUISystem in iOnGUISystems)
                {
                    try
                    {
                        iOnGUISystem.Run(component);
                    }
                    catch (Exception e)
                    {
                        Log.Error(e);
            ...
            ObjectHelper.Swap(ref this.onguis, ref this.onguis2);
        }
