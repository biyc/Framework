...
namespace UniRx.WebRequest
{
    public static class ObservableWebRequest
    {
            ...
            return
                ObservableUnity.FromCoroutine<string>(
                    (observer, cancellation) =>
                        FetchText(unityWebRequest, headers, observer, progress, cancellation));
        }
            ...
            IProgress<float> progress = null)
        {
            ...
            request.timeout = 5;
            return ObservableUnity.FromCoroutine<byte[]>((observer, cancellation) =>
                FetchBytes(request, headers, observer, progress, cancellation));
        }
            ...
            IDictionary<string, string> headers = null, IProgress<float> progress = null)
        {
            ...
            return ObservableUnity.FromCoroutine<string>((observer, cancellation) =>
                FetchText(request, headers, observer, progress, cancellation));
        }

        public static IObservable<byte[]> PostData(string url, byte[] data,
            IDictionary<string, string> headers = null, IProgress<float> progress = null)
        {
            var request = new UnityWebRequest(url, "POST");
            var bodyByte = data;
        ...
        {
            return
                null; // ObservableUnity.FromCoroutine<AssetBundle>((observer, cancellation) => FetchAssetBundle(UnityWebRequest.GetAssetBundle(url, version, crc),null, observer, progress, cancellation));
        }
